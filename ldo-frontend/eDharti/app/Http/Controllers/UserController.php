...
class UserController extends Controller
{
    ...
    public function getUserList(Request $request)
    {
        $query = User::query()
            ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
            ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
            ->select('users.id', 'users.name', 'users.email', 'users.status', 'users.user_type')
            ->selectRaw('GROUP_CONCAT(roles.name SEPARATOR ", ") as role_names')
            ->groupBy('users.id', 'users.name', 'users.email', 'users.status', 'users.user_type'); // Add all necessary columns here
        ...
        foreach ($userData as $row) {
            ...
            // Prepare roles as a comma-separated list
            $roles = !empty($row->role_names) ? $row->role_names : ''; // Prepare the roles
            $nestedData['roles'] = '<label class="badge bg-primary mx-1">' . $roles . '</label>';
