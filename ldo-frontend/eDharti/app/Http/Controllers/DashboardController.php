...
class DashboardController extends Controller
{
    ...
    private function getSectionData()
    {
        ...
        $getUserRegistrationCountData = DB::select('CALL GetUserRegistrationCounts()');
        $data['registrations'] = [
            'totalCount' => $getUserRegistrationCountData[0]->TotalCount,
            'newCount' => $getUserRegistrationCountData[0]->RS_NEW,
            'appCount' => $getUserRegistrationCountData[0]->RS_APP,
            'rejCount' => $getUserRegistrationCountData[0]->RS_REJ,
            'urewCount' => $getUserRegistrationCountData[0]->RS_UREW,
            'rewCount' => $getUserRegistrationCountData[0]->RS_REW,
            'penCount' => $getUserRegistrationCountData[0]->RS_PEN,
        ];
        $getNewPropertyCountData = DB::select('CALL GetNewPropertyCounts()');
        $data['newProperty'] = [
            'totalCount' => $getNewPropertyCountData[0]->TotalCount,
            'newCount' => $getNewPropertyCountData[0]->RS_NEW,
            'appCount' => $getNewPropertyCountData[0]->RS_APP,
            'rejCount' => $getNewPropertyCountData[0]->RS_REJ,
            'urewCount' => $getNewPropertyCountData[0]->RS_UREW,
            'rewCount' => $getNewPropertyCountData[0]->RS_REW,
            'penCount' => $getNewPropertyCountData[0]->RS_PEN,
        ];
        ...
        foreach ($sections as $section) {
            array_push($sectionCodes, $section->section_code);
        }
        $applicationData = $this->getApplicationData($sectionCodes);
        $data = array_merge($data, $applicationData);
        $data['statusList'] = getApplicationStatusList(true);
        return $data;
    }
    ...
    public function getApplicationData($sectionCodes)
    {
        ...
        $statusWiseCounts = $this->combineStatusData($mutationData, $doaData, $lucData);

        // Calculate total application count by summing status-wise counts
        $totalAppCount = array_sum($statusWiseCounts);
        //to be changed
        return ['mutataionData' => $mutationData, 'doaData' => $doaData, 'lucData' => $lucData, 'totalAppCount' => $totalAppCount, 'statusWiseCounts' => $statusWiseCounts];
    }
