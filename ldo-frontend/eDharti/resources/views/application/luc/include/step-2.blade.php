                                ...
                                <div class="col-lg-4">
                                    <div class="form-group form-box">
                                        ...
                                        <input type="file" name="{{$doc['id']}}" class="form-control"
                                            accept="application/pdf" id="{{$doc['id']}}" onchange="handleFileUpload(this.files[0], &quot;{{$doc['label']}}&quot;,'LUC','LUC')">
                                    ...
                                    $showViewLink = false;
                                    $finalDocument = null;
                                    if(isset($stepSecondFinalDocuments) && isset($stepSecondFinalDocuments[$key]))
                                    {
                                    $finalDocument = $stepSecondFinalDocuments[$key];
                                    $showViewLink = true;
                                    }
                                    @endphp
                                    <a href="{{!is_null($finalDocument) ? asset('storage/' .$finalDocument['file_path'] ):''}}" target="_blank" class="fs-6 {{$showViewLink ? '':'d-none'}}">View saved document</a>
                                </div>
                                @endforeach
                            </div>
                        </div>
